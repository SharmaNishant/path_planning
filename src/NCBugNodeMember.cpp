                ...
                vector< geometry_msgs::Point > &pathList, LocationList &locationList, bool &killFlag)
{
    ...
    {
        Point nextPoint;
        double angleGoal = atan2(goal.y - bugList[i].getCurPos().y, goal.x - bugList[i].getCurPos().x);
    ...
    if(lineFlag && minDist <= bugStepSize)
    {
        ...
        else
        {
            ...
            //cout<<"location updated : total "<<locationList.getLocationList().size()<<endl;
        ...
        newPos.x = lineIds[intersectionPointID].x;
        newPos.y = lineIds[intersectionPointID].y;
        NCBug newTempBug(boundaryFollowing, newPos, bugList[i].getDistance(), bugList[i].getLastPointID(), bugList[i].getBoundaryID(), bugList[i].getPath());
    ...
    if(!lineFlag)
    {
        ...
        float distanceTemp = bugList[i].getEuclideanDistance(bugList[i].getCurPos(),goal);
...
void boundaryFollow(vector<NCBug> &bugList,int i, Point goal, vector < NCOLine > &obstacleLines, vector< vector< Point > > &obstArray,
                    vector< geometry_msgs::Point > &pathList, LocationList &locationList, bool &killFlag)

{
    ...
    float distanceTempGoal = bugList[i].getEuclideanDistance(bugList[i].getCurPos(),bugList[i].getTempGoal());
    ...
    {
        Point nextPoint;
        double angleGoal = atan2(bugList[i].getTempGoal().y - bugList[i].getCurPos().y, bugList[i].getTempGoal().x - bugList[i].getCurPos().x);
        nextPoint.x = bugList[i].getCurPos().x + (bugStepSize * cos(angleGoal));
        nextPoint.y = bugList[i].getCurPos().y + (bugStepSize * sin(angleGoal));

        //check if inside the obstacle (we are not doing it because we are on a line)
        ...
        if(lineIds.size() == 0)
        {
            if(!bugList[i].checkInsideObstacle(obstArray,goal))
            {
                ...
                else
                {
                    int tempID = locationList.addLocation(loc);
                    bugList[i].setLastPointID(tempID);
                    //cout<<"location added leaving for goal : total "<<locationList.getLocationList().size()<<endl;
                    //ros::Duration(1).sleep();
                    //cout<<"temp id " << tempID << endl;
                }
    ...
    else // distanceTempGoal < bugStepSize
    {
        ...
        //add this point in the graph
        int locID = locationList.searchLocation(bugList[i].getCurPos().x, bugList[i].getCurPos().y);
        if(locID != -1)
        {
        ...
        else
        {
           // cout<<loc.getPrevNode()<<" i "<<i<<endl;
            int tempID = locationList.addLocation(loc);
            bugList[i].setLastPointID(tempID);
            //cout<<"location added line end : total "<<locationList.getLocationList().size()<<endl;
            //ros::Duration(1).sleep();
        }
        ...
        vector< NCOLine > pathLines = bugList[i].getPath();
        lineIds = bugList[i].getLineIntersections(goal, pathLines);
        if(bugList[i].checkInsideObstacle(obstArray, goal) || lineIds.size() != 0)
        {
