...
bool addNewPointtoRRT(RRT &myRRT, RRT::rrtNode randPointNode, int rrtStepSize,
                      vector<vector<geometry_msgs::Point>> &obstArray,
                      visualization_msgs::Marker &rrtTreeMarker,
                      vector<vector<int>> &rrtPaths) {
  // ROS_INFO("Entering getKnearestNodes");
  vector<int> nearestNodeID =
      myRRT.getKNearestNodesID(randPointNode.posX, randPointNode.posY, kValue);
  ...
  while (i < kValue) {
    if (nearestNodeID[i] == -1)
      break;
    RRT::rrtNode nearestNode = myRRT.getNode(nearestNodeID[i]);

    double theta = atan2(randPointNode.posY - nearestNode.posY,
                         randPointNode.posX - nearestNode.posX);
