...
void Bug::setState(State state) { this->state = state; }
...
void Bug::setLastX(float x) { this->lastX = x; }

void Bug::setLastY(float y) { this->lastY = y; }

void Bug::setLastPointID(int Id) { this->lastPointID = Id; }

void Bug::setBoundaryID(int boundaryId) { this->boundaryID = boundaryId; }

void Bug::setDistance(float dist) { this->distance = dist; }

void Bug::addDistance(float dist) { this->distance += dist; }

/**
    Get Functions
*/

State Bug::getState() { return this->state; }

int Bug::getID() { return this->ID; }

float Bug::getX() { return this->x; }

float Bug::getY() { return this->y; }

float Bug::getLastX() { return this->lastX; }

float Bug::getLastY() { return this->lastY; }

int Bug::getBoundaryID() { return this->boundaryID; }

float Bug::getDistance() { return this->distance; }

int Bug::getLastPointID() { return this->lastPointID; }
...
/*
void Bug::moveBug(float goalX, float goalY, vector < obstacleLine >
&obstacleLines, vector< vector<geometry_msgs::Point> > &obstacleList, int
&pointID, int &pointIDOne,int &pointIDTwo, LocationArray &locationList, bool
&killFlag, Bug** newBug)
{
    ...
    if(this->getState() == movingToGoal)
    {
        ...
        {
            if((this->getX() == lineIds[j].x) && (this->getY() == lineIds[j].y))
continue;

            dist = getEuclideanDistance(this->getX(),this->getY(),
lineIds[j].x,lineIds[j].y);
        ...
        if(lineFlag)
        {
            ...
            int locID =
locationList.searchLocation(lineIds[intersectionPointID].x,lineIds[intersectionPointID].y);
            if(locID != -1)
            {
            ...
            Bug* newTempBug = new Bug(boundaryFollowing,
lineIds[intersectionPointID].x, lineIds[intersectionPointID].y,
this->getDistance(), this->getLastPointID(), this->getBoundaryID());
            ...
            locID = locationList.searchLocation(newTempBug->getX(),
newTempBug->getY()); if(locID != -1)
            {
                float prevDistanceToPoint = locationList.getDistance(locID);
                if(prevDistanceToPoint < loc.getDistance())
                {
...
bool Bug::checkInsideObstacles(vector<vector<geometry_msgs::Point>> &obstArray,
...
int Bug::checkIfOnOtherLines(vector<obstacleLine> &obstacleLines, int &index) {
  for (int i = 0; i < obstacleLines.size(); i++) {
    ...
    if (obstacleLines[i].point[0].x == this->getX() &&
        obstacleLines[i].point[0].y == this->getY()) {
      index = 1;
      return i;
    }
    if (obstacleLines[i].point[1].x == this->getX() &&
        obstacleLines[i].point[1].y == this->getY()) {
...
Bug::getLineIntersections(float goalX, float goalY,
...
float Bug::getEuclideanDistance(float p0_x, float p0_y, float p1_x,
                                float p1_y) {
