...
void dijkstra::getShortestPath(int source, int destination,
                               vector<int> &shortestPath) {
  this->runDijkstra(source);
  while (path[destination] != -1) { // If we are not at the source node
...
void dijkstra::setGraph(float **graph) { this->graph = graph; }

float **dijkstra::getGraph() { return this->graph; }
