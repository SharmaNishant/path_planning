...
NCBug::NCBug(State state, Point loc, float dist, int lastPTID, int boundID, vector< NCOLine > path)
{
    this->state = state;
    this->curPos.x = loc.x;
    this->curPos.y = loc.y;
...
void NCBug::setState(State state)
{
    this->state = state;
}
...
void NCBug::setLastPos(Point pos) {
    this->lastPos = pos;
}

void NCBug::setTempGoal(Point pos) {
    this->tempGoal = pos;
}

void NCBug::setLastPointID(int Id)
{
    this->lastPointID = Id;
}

void NCBug::setBoundaryID(int boundaryId)
{
    this->boundaryID = boundaryId;
}

void NCBug::setDistance(float dist)
{
    this->distance = dist;
}

void NCBug::addDistance(float dist)
{
    this->distance += dist;
}

void NCBug::setPath(vector<NCOLine> path)
{
    this->visitedPath = path;
}

void NCBug::addPath(NCOLine line)
{
    this->visitedPath.push_back(line);
}
...
Point NCBug::getCurPos()
{
    return this->curPos;
}

Point NCBug::getLastPos()
{
    return this->lastPos;
}

Point NCBug::getTempGoal()
{
    return this->tempGoal;
}

int NCBug::getBoundaryID()
{
    return this->boundaryID;
}

float NCBug::getDistance()
{
    return this->distance;
}

int NCBug::getLastPointID()
{
    return this->lastPointID;
}

vector< NCOLine > NCBug::getPath()
{
...
{
    double theta = atan2(goal.y - this->getCurPos().y,goal.x - this->getCurPos().x);
...
int NCBug::checkIfOnOtherLines(vector < NCOLine > &obstacleLines)
{
    for(int i=0; i< obstacleLines.size(); i++)
    {
        ...
        if(obstacleLines[i].start.x == this->getCurPos().x && obstacleLines[i].start.y == this->getCurPos().y)
        {
        ...
        if(obstacleLines[i].end.x == this->getCurPos().x && obstacleLines[i].end.y == this->getCurPos().y)
        {
            //set which direction it should move in now
...
vector<IntersectionPoint> NCBug::getLineIntersections(Point goal, vector < NCOLine > &obstacleLines)
{
