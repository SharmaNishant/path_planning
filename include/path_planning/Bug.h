...
public:
  location();
  location(int prev, int curr, int next, float x, float y, float distance,
           int bugID);
...
public:
  ...
  /** Actual Bug Function */
  // void moveBug(float goalX, float goalY, vector < obstacleLine >
  // &obstacleLines, vector< vector<geometry_msgs::Point> > &obstacleList,
  //             int &pointID, int &pointIDOne,int &pointIDTwo, LocationArray
  //             &locationList, bool &killFlag, Bug** newbug);
  ...
  getLineIntersections(float goalX, float goalY,
